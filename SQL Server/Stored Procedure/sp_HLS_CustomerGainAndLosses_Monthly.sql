...
/****** Object:  StoredProcedure [dbo].[sp_HLS_CustomerGainAndLosses_Monthly]    Script Date: 08/25/2014 12:16:52 ******/
SET ANSI_NULLS ON
GO
...
07/23/2014 			Jide Akintoye				Create HLS Monthly Customer Gain and Losses		


**********************************************************************************************/

CREATE PROCEDURE [dbo].[sp_HLS_CustomerGainAndLosses_Monthly]
    ( @StartDate DATETIME ,
      @EndDate DATETIME ,
      @State NVARCHAR(MAX) ,
      @Status NVARCHAR(MAX)
...
****************************************************/
        SELECT  'MonthID' = DATEPART(mm , EnrollDate) ,
                'Month' = DATENAME(mm , EnrollDate) + ' ' + DATENAME(YY ,
                                                              EnrollDate) ,
                'MonthlyGain' = SUM(DailyGainCount) ,
                'MonthlyLoss' = SUM(DailyCancelCount) ,
                'MonthlyNetGains' = SUM(DailyGainCount) - SUM(DailyCancelCount)
        FROM    #TempTable1
        WHERE   EnrollDate >= @StartDate
                AND EnrollDate <= @EndDate
        GROUP BY DATENAME(mm , EnrollDate) ,
