...
/****** Object:  StoredProcedure [dbo].[sp_MvInMvOt_GainsChart]    Script Date: 08/25/2014 12:21:39 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[sp_MvInMvOt_GainsChart]
(@StartDate DATETIME
,@EndDate DATETIME
--, @State NVARCHAR (MAX) 
--, @Status NVARCHAR (MAX) 
...
---------------------------------   
    ...
    SELECT  MONTHID
          , BeginServiceMonth
          , CASE WHEN EnrollType = 'NewEnroll' THEN SUM(RecordCount)
                 ELSE NULL
            END AS NewEnroll
          , CASE WHEN EnrollType = 'TOS' THEN SUM(RecordCount)
                 ELSE NULL
            END AS TOS
          , CASE WHEN EnrollType = 'Recycle1' THEN SUM(RecordCount)
                 ELSE NULL
            END AS Recycle1
          , CASE WHEN EnrollType = 'Recycle2' THEN SUM(RecordCount)
                 ELSE NULL
            END AS Recycle2
    FROM    #MvInMvOt_Summary
    GROUP BY MONTHID, EnrollType
          , BeginServiceMonth
