...
/****** Object:  StoredProcedure [dbo].[sp_Customer_Profile_CIS2_GA_backup]    Script Date: 08/25/2014 09:51:28 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[sp_Customer_Profile_CIS2_GA_backup]
AS
    PRINT 'Drop Tables'
    DROP TABLE #cis2
...
--c.RecordLastUpdatedDate
    ...
    SELECT  c.LDCNo ,
            ...
            c.scenario ,
            DATEDIFF(DAY , c.BeginServiceDate , GETDATE()) diff
    FROM    #cis2 c --left join 
    FULL OUTER JOIN CustomerProfile_GA_TransData_v v ON c.LDCNo = v.LDC_ACCOUNT_NUMBER
    WHERE   c.BeginServiceDate IS NULL
            AND LDCNo IS NOT NULL
...
--LEFT(c.PremStatusID,2) not in ('01','02','03','05')
    ...
    WHERE   RecordLastUpdatedBy = 'sp_Customer_Profile_Load_CIS2_GA'
