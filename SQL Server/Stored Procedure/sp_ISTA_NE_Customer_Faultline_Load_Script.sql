...
/****** Object:  StoredProcedure [dbo].[sp_ISTA_NE_Customer_Faultline_Load_Script]    Script Date: 08/25/2014 12:21:09 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[sp_ISTA_NE_Customer_Faultline_Load_Script]
AS
    BEGIN
...
---------------------------------
--SELECT #Customer.PremID, #Customer.PremNo, #Customer.CustId, Min(v_Contract.RateDetID) as RateDetID -- Add PremNo and CustId (sn 1/27/14)
--Into #CurrContract
--FROM #Customer
--       LEFT OUTER JOIN StreamInternal.dbo.v_Contract v_Contract
--         ON (#Customer.PremID = v_Contract.PremID AND #Customer.PremNo = v_Contract.PremNo AND #Customer.CustId = v_Contract.CustId   --Add PremNo and CustId to join (sn 1/27/14)
--         AND isnull(#Customer.EndServiceDate, GETDATE()) >= v_Contract.EffectiveDate and isnull(#Customer.EndServiceDate, GETDATE())
--             <= isnull (v_Contract.ExpirationDate, GETDATE()) and v_Contract.Active = 1)
-- GROUP BY #Customer.PremID, #Customer.PremNo, #Customer.CustId 
