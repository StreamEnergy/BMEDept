...
/****** Object:  StoredProcedure [dbo].[sp_MvInMvOt_DropChart]    Script Date: 08/25/2014 12:21:32 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[sp_MvInMvOt_DropChart]
(@StartDate DATETIME
,@EndDate DATETIME
--, @State NVARCHAR (MAX) 
--, @Status NVARCHAR (MAX) 
...
---------------------------------   
    ...
    SELECT  MONTHID
          , EndServiceMonth
          , CASE WHEN DropType = 'New-Drop' THEN SUM(RecordCount)
                 ELSE NULL
            END AS [New-Drop]
          , CASE WHEN DropType = 'TOS-Drop' THEN SUM(RecordCount)
                 ELSE NULL
            END AS [TOS-Drop]
          , CASE WHEN DropType = 'Recycle1-Drop' THEN SUM(RecordCount)
                 ELSE NULL
            END AS [Recycle1-Drop]
          , CASE WHEN DropType = 'Recycle2-Drop' THEN SUM(RecordCount)
                 ELSE NULL
            END AS [Recycle2-Drop]
    FROM    #MvInMvOt_DropSummary
    GROUP BY MONTHID, DropType
          , EndServiceMonth
