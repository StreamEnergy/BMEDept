...
/****** Object:  StoredProcedure [dbo].[sp_CP_Reload_Customer_Count]    Script Date: 08/25/2014 09:50:25 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[sp_CP_Reload_Customer_Count]
AS
    BEGIN
...
----------------------------------------------------
        ...
        TRUNCATE TABLE  [StreamInternal].[dbo].[CP_Customer_Count];
        ...
        INSERT  INTO #Temp
                SELECT  ( CASE WHEN c.State = 'TX' THEN 'Texas'
                        ...
                        ( SELECT    SUM(cc.GainCount)
                          FROM      [StreamInternal].[dbo].[CP_Customer_Count] cc
                        ...
                        ( SELECT    SUM(c2.LossCount)
                          FROM      [StreamInternal].[dbo].[CP_Customer_Count] c2
                        ...
                        ( SELECT    SUM(c3.NetCount)
                          FROM      [StreamInternal].[dbo].[CP_Customer_Count] c3
                        ...
                        ( SELECT    SUM(c4.GainCount) - SUM(c4.LossCount)
                          FROM      [StreamInternal].[dbo].[CP_Customer_Count] c4
                        ...
                        ( SELECT    SUM(c5.GainCount) - SUM(c5.LossCount)
                          FROM      [StreamInternal].[dbo].[CP_Customer_Count] c5
...
--'darren.williams@streamenergy.net;steve.nelson@streamenergy.net;mark.cheng@streamenergy.net;matt.baker@streamenergy.net',
            @subject = 'Customer Counts' , @body = @tableHTML ,
            @body_format = 'HTML';
