...
/****** Object:  StoredProcedure [dbo].[sp_Customer_Profile_Load_ISTA_NE]    Script Date: 08/25/2014 09:53:34 ******/
SET ANSI_NULLS ON
GO
...
02/13/2014				Matt Baker							Adjusted Current Product Logic to compensate for gaps in contract expiration date and altered End Service 
															date to shave off time from get date so fractional time will not be an issue.
02/19/2014				Darren Williams						Changed table from Test to Prod
...
CREATE PROCEDURE [dbo].[sp_Customer_Profile_Load_ISTA_NE]
AS
    BEGIN
...
-------------------------------
        ...
        LEFT OUTER JOIN StreamInternal.dbo.v_Contract v_Contract ON ( #Customer.PremID = v_Contract.PremID
                                                              ...
                                                              CAST(GETDATE() AS DATE)) >= v_Contract.EffectiveDate
                                                              AND ISNULL(#Customer.EndServiceDate ,
                                                              CAST(GETDATE() AS DATE)) <= ISNULL(v_Contract.ExpirationDate ,
                                                              '12/31/2999')
...
-------------------------------
        BEGIN TRY
            ...
            BEGIN TRANSACTION NE

            DELETE  FROM [StreamInternal].[dbo].[CustomerProfile]
            WHERE   [DataSource] = 'ISTA'; 

            INSERT  INTO [StreamInternal].[dbo].[CustomerProfile] --[tmp_CustomerProfile_ISTA]
                    ( CustID ,
                      CustNo ,
                      PremID ,
                      PremNo ,
		...
		--top 10
                            ...
                            'ISTA' AS DataSource ,
                            'sp_Customer_Profile_Load_ISTA_NE' AS RecordCreatedBy ,
                            CONVERT(DATETIME , GETDATE() , 120) AS RecordDate ,
                            'sp_Customer_Profile_Load_ISTA_NE' AS RecordLastUpdatedBy ,
...
--       RIGHT OUTER JOIN #Customer ON (#Customer.PremID = #CurrContract.PremID)
            IF @@TRANCOUNT > 0
                BEGIN --SUCCESS, nothing failed, now I can commit!!
                    COMMIT TRANSACTION NE;    -- now everything is committed
                END
      --SET @IsError = @@ERROR
            ...
            IF @@TRANCOUNT > 0
                BEGIN
                    ...
                    EXEC msdb.dbo.sp_send_dbmail @profile_name = 'SMTPRELAY01' ,
                        ...
                        @subject = 'Error in Customer Profile for NE' ,
                        @body = 'There was error on insert data into CustomerProfile in sp_Customer_Profile_Load_ISTA_NE' ,
                        @body_format = 'HTML';        
                END
...
---------------------------------	
        ...
        IF @RecordDate < CONVERT(VARCHAR(10) , GETDATE() , 110)
            BEGIN
                SET @tableHTML = N'<br>'
                    + N'The Customer Profile for NE does not have latest data, please check in the sp_Customer_Profile_Load_ISTA_NE'
                    + N'NE data is more than 1 day old.  It should only be reloaded on current date.'
                    + N'<br><br><br>' + N'<table>'
