...
/****** Object:  View [dbo].[v_EAGLE_Free_Gas_50]    Script Date: 08/26/2014 13:51:08 ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dbo].[v_EAGLE_Free_Gas_50]
AS
    SELECT  'Free Gas 50' AS Program_Name ,
    ...
    INNER JOIN ( SELECT ADcode
                 FROM   StreamInternal.dbo.v_EAGLE_HighwayGas WITH ( NOLOCK )
                 GROUP BY ADcode
                 HAVING MAX(CustNumber) > 9
    ...
    INNER JOIN EAGLE.dbo.tblCustomers tc WITH ( NOLOCK ) ON tc.[Sponsor Number] = td.[IA Number]
                                                            ...
                                                            'Pending' ,
                                                            'Active' )
    INNER JOIN StreamInternal.dbo.v_EAGLE_HighwayGas fe WITH ( NOLOCK ) ON fe.Cdcode = tc.[Customer Number]
    WHERE   tc.[App Date] > '2013-09-28 00:00:00.000'
            AND td.DStatusDesc = 'Active'
            AND td.Type = 'D'
            AND td.[IA Number] NOT IN ( SELECT DISTINCT
                                                [IA_Number]
                                        FROM    dbo.v_EAGLE_Free_Gas_100 )
            AND td.[IA Number] NOT IN (
            SELECT DISTINCT
                    [IA_Number]
            FROM    dbo.v_EAGLE_Free_Gas_100_Ignition )
